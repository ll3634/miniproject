...
if args.resume:
    ...
    checkpoint = torch.load('./checkpoint/ckpt_new_after_trick.pth')
...
def valid(epoch):
    ...
    with open('acc_ResNet50_F_after_trick.txt','a+')as f:
        f.write('%.5f'%acc)
        f.write('\n')
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/ckpt_new_after_trick.pth')
        best_acc = acc
    return acc
...
#----------------------------------------------------------------------add start
    ...
    net.load_state_dict(torch.load('./checkpoint/ckpt_new_after_trick.pth')['net'])
#----------------------------------------------------------------------add finish  
    net.eval()
    test_loss = 0
    correct = 0
